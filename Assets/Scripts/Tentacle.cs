...
public class Tentacle : MonoBehaviour {
	...
	void Update() 
	{
		Vector2[] temp = new Vector2[_tentacleData.nodes.Count];

		for (int i = 0; i < temp.Length; i++) {
			temp [i] = _tentacleData.nodes[i].GetVec();
		}
		m_EdgeCollider.points = temp;

		m_LineRenderer.positionCount = temp.Length;
		m_LineRenderer.SetPositions (Vector2toVector3 (temp));
		// UpdateParts() ;
	...
	Vector3[] Vector2toVector3(Vector2[] array)
	{
		Vector3[] Vec3 = new Vector3[array.Length];
		for (int i = 0; i < array.Length; i++) 
		{
			Vec3[i] = new Vector3 (array[i].x,array[i].y, 0f);
		}
		return Vec3;
	}
}