...
public class Water : MonoBehaviour {
	...
	public float p_left = -10;
	public float p_width = 20;
	public float p_top = 0;
	public float p_bottom = -3;
	...
	{
		//Spawning our water
		SpawnWater(p_left+transform.position.x,p_width,p_top+transform.position.y,p_bottom+transform.position.y);
	...
	public void SpawnWater(float Left, float Width, float Top, float Bottom)
	{
		...
		gameObject.GetComponent<BoxCollider2D>().offset = new Vector2(Left-transform.position.x + Width / 2, (Top-transform.position.y + Bottom-transform.position.y) / 2);
		...
		for (int i = 0; i < edgecount; i++)
		{
			...
			meshobjects [i].transform.position = new Vector3(0f, 0f, 0f);
	...
	void OnTriggerStay2D(Collider2D Hit)
	{
		...
		int layer = 1 << 4; // layer 4 - Water layer
		if (Hit.tag == "Player" && lm == layer) 
		{
			//Hit.GetComponent<Rigidbody2D> ().velocity = Vector2.zero;

			foreach (GameObject m in meshobjects) 
			{
				Material[] tempArray = { matTransparent };

				m.GetComponent<MeshRenderer> ().materials = tempArray;
			}
			/*
			...
			foreach (GameObject m in meshobjects) 
			{
				if (Vector2.Distance (Hit.transform.position, m.transform.GetChild (0).transform.position) < Hit.GetComponent<Renderer> ().bounds.size.magnitude) {
